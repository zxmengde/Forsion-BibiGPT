...
export const AnalyticsProvider: React.FC<{ children: React.ReactElement }> = ({ children }) => {
  ...
  const analytics = React.useMemo(() => {
    // Only load Analytics if writeKey is provided
    if (!writeKey) {
      isDev && console.log('AnalyticsBrowser skipped: no writeKey provided')
      // Return a mock analytics object to prevent errors
      return {
        track: () => {},
        page: () => {},
        identify: () => {},
